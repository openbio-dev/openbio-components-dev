...
var WS = /** @class */ (function () {
    ...
    WS.prototype.init = function () {
        var _this = this;
        console.info("websocket will try connection", hooks().format());
        try {
        ...
        this.deviceSocket.addEventListener("close", function () {
            _this.deviceSocket.close();
            console.warn("device websocket connection closed", hooks().format());
        });
        this.deviceSocket.addEventListener("error", function () {
            _this.deviceSocket.close();
        });
        this.deviceSocket.addEventListener("open", function () {
            console.info("device websocket connection opened", hooks().format());
        });
        this.componentSocket = new WebSocket("ws://" + constants.WS_HOST + "/component");
        this.componentSocket.addEventListener("close", function () {
            _this.componentSocket.close();
        });
        this.componentSocket.addEventListener("error", function () {
            _this.componentSocket.close();
        });
        this.componentSocket.addEventListener("open", function () {
            console.info("component websocket connection opened", hooks().format());
        });
        this.interfaceSocket = new WebSocket("ws://" + constants.WS_HOST + "/interface");
        this.interfaceSocket.addEventListener("close", function () {
            _this.interfaceSocket.close();
        });
        this.interfaceSocket.addEventListener("open", function () {
            console.info("interface websocket connection opened", hooks().format());
        });
        this.interfaceSocket.addEventListener("error", function () {
            _this.interfaceSocket.close();
        });
    };
    WS.prototype.respondToDeviceWS = function (message) {
    ...
    WS.prototype.respondToComponentWS = function (message) {
        this.componentSocket.send(JSON.stringify(message));
    };
    WS.prototype.changeModuleTo = function (message) {
        this.interfaceSocket.send(JSON.stringify(message));
    };
    WS.prototype.status = function () {
