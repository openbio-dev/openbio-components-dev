...
import { loadFingerForm, loadPalmForm } from './api';
...
export class OpenbioSignatureComponentDetails {
    ...
    getDeviceList() {
        this.payload.action = "scanner-get-device-list";
        this.ws.respondToDeviceWS(this.payload);
    }
    setDevice() {
    ...
    capture() {
        if (!this.canCapture()) {
            notify(this.componentContainer, "warning", "Realize a inicialização do dispositivo antes de capturar");
            return;
        }
    ...
    clearRects() {
        ...
        ctx.drawImage(this.form.fileData.image, 0, 0, this.form.fileData.image.width, this.form.fileData.image.height, 0, 0, this.canvas.width, this.canvas.height);
    }
    loadForm(file) {
    ...
    setFormTypeSelection(event) {
        ...
        this.palmType = PALM_TYPES.UNDEFINED;
        this.form.canvas.height = this.formType === FORM_TYPE.DECADACTILAR ? 225 : 500;
        console.log("SELECTED FORM TYPE", this.formType);
    }
    setPalmarTypeSelection(event) {
        this.palmType = parseInt(event.target.value, 10);
        console.log("SELECTED PALM TYPE", this.palmType);
    }
    setAutoNextSelection(event) {
        this.form.autoNext = event.target.checked;
        console.log("AUTO NEXT SELECTION", this.form.autoNext);
    }
    startDevice() {
        if (!this.selectedDevice) {
            notify(this.componentContainer, "warning", "Selecione um dispositivo.");
            return;
        }
    }
    resetForm() {
    ...
    canCapture() {
        return (this.formType === FORM_TYPE.DECADACTILAR || (this.formType === FORM_TYPE.PALMAR && this.palmType !== PALM_TYPES.UNDEFINED)) && this.deviceReady;
    }
    canUpload() {
    ...
    showFingerSelectionRect(finger) {
        ...
        if (this.canvas && this.canvas.getContext) {
            const ctx = this.canvas.getContext('2d');
            ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);
            ctx.drawImage(this.form.fileData.image, 0, 0, this.form.fileData.width, this.form.fileData.height, 0, 0, this.canvas.width, this.canvas.height);
    ...
    isPalmCaptured(type) {
        console.log(type, this.form.palms);
    ...
    render() {
        const deviceList = (this.deviceList || []).map((device) => {
            return (h("option", { value: device.name, selected: this.selectedDevice === device.name }, device.name));
        });
        return (h("div", { class: "window-size" },
            ...
            h("div", { class: "columns is-mobile" },
                h("div", { class: "column is-half" },
                    ...
                    h("div", { class: "mb-10 columns scanner-device-container" },
                        h("div", { class: "mt-10 ml-10 select is-small" },
                            h("select", { onChange: this.setDeviceSelection.bind(this) },
                                h("option", { value: undefined }, " Selecione o dispositivo de captura "),
                                deviceList)),
                        h("div", { class: "column" },
                            h("a", { class: `button is-small action-button ${this.selectedDevice ? "" : "disabled"}`, title: `${this.selectedDevice ? "Clique aqui para inicializar o dispositivo" : "Selecione um dispositivo antes de iniciar"}`, onClick: () => this.startDevice() }, "Inicializar dispositivo"))),
                    h("div", { class: "mb-10" },
